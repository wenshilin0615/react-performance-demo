...
function DemoD() {
  ...
  return (
    <Profiler id="DemoD" onRender={onRenderCallback}>
      ...
      <h2>Demo D - React 16 自动批处理演示</h2>
      <p>React 16 仅在事件处理器中批处理，setTimeout/Promise 中每次 setState 都触发渲染</p>
      ...
      <div className="demo-d-stats">
        ...
        <div><span>渲染次数:</span><strong style={{ color: '#d32f2f' }}>{renderLog.current.length}</strong></div>
      </div>
      ...
      <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#ffebee', borderRadius: '4px' }}>
        <strong>React 16 特点：</strong>
        <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
          <li>setTimeout/Promise 中：3 次 setState = 3 次渲染</li>
          <li>事件处理器中：3 次 setState = 1 次渲染（自动批处理）</li>
        </ul>
      </div>
      </div>
    </Profiler>
